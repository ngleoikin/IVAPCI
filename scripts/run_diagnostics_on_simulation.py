...
def _build_estimator(
    ...
    if name == "ivapci_v3_3_hier":
        ...
        cfg = IVAPCIV33TheoryConfig(
            x_dim=x_dim, w_dim=w_dim, z_dim=z_dim, n_samples_hint=n_samples
        )
        if seed is not None:
            cfg.seed = seed
...
def _normalize_scenarios(raw: List[str]) -> List[str]:
    ...
    for token in raw:
        ...
        for part in parts:
            ...
            if pos != len(part):
                guidance = "Ensure scenario names are separated by spaces or commas."
                if "--" in part:
                    guidance += (
        ...
        for seed in seed_list:
            ...
            for method in methods:
                ...
                row = {
                    ...
                    "protected_features": train_diag.get("protected_features", np.nan),
                    "info_loss_proxy": train_diag.get("info_loss_proxy", np.nan),
            ...
            if "pacdt_v3_0" in latent_store:
                ...
                if iv_latent is None:
                    iv_latent = latent_store.get("ivapci_v3_2_hier")
                if iv_latent is None:
                    iv_latent = latent_store.get("ivapci_v3_2_hier_radr")
                if iv_latent is None:
